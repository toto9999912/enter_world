...
namespace Core
{
    ...
    public class GameServicesInitializer : MonoBehaviour
    {
        ...
        private static GameServicesInitializer instance;
        ...
        private void RegisterCoreServices()
        {
            ...
            // 2. 背包服務 (需要先創建 InventorySystem 實例)
            var inventorySystem = new InventorySystem(initialCapacity: 50);
            var inventoryService = new InventoryService(inventorySystem);
        ...
        private void SubscribeToGlobalEvents()
        {
            ...
            LogDebug("[EventBus] 已訂閱 ItemObtainedEvent");
        }
        ...
        {
            LogDebug($"[全域事件] 造成傷害: {e.Damage:F1} ({e.DamageType})");
        }

        private void OnEntityDeath(EntityDeathEvent e)
        {
            LogDebug($"[全域事件] 實體死亡: {e.Entity?.name}");
        }

        private void OnItemObtained(ItemObtainedEvent e)
        {
            LogDebug($"[全域事件] 獲得物品: {e.Item?.itemName} x{e.Amount}");
        }
